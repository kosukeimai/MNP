...
\author{Kosuke Imai\thanks{Assistant Professor, Department of
    ...
    609--258--6610, Fax: 973--556--1929, Email:
    \href{mailto:kimai@Princeton.Edu}{kimai@Princeton.Edu}, URL:
    \href{http://imai.princeton.edu}{http://imai.princeton.edu}
    Department of Politics, Princeton University}\\
  David A. van Dyk\thanks{Associate Professor, Department of
    Statistics, University of California, Irvine, CA 92697-1205.
    Phone: 949--824--5679, Fax: 949--824--9863, Email:
    \href{mailto:dvd@uci.edu}{dvd@uci.edu}}}
...
model (In addition, \texttt{coef()} and \texttt{cov.mnp()} allow one
...
Note that \texttt{coef(res)} and \texttt{cov.mnp(res)} allow one to
...
To use MNP, you must install the statistical software R (if it is not
already installed) as well as the MNP package. 

\subsection{Windows systems}

\begin{enumerate}
\item {\bf Installing the latest version of R. } You may skip this
  step if the latest version of R is already installed on your system.
  If R is not installed on your system, go to the Comprehensive R
  Archive Network (CRAN) website
  (\href{http://cran.r-project.org}{http://cran.r-project.org}) and
  download the latest R installer for Windows.  Double-click on the
  {\tt .exe} file to launch the installer.  We recommend that you
  accept the default installation options.

\item {\bf Installing MNP.} Start R and type at the prompt:
\spacingset{1}\begin{verbatim}
...
\subsection{Unix/Linux systems}

\begin{enumerate}
\item {\bf Installing the latest version of R.} You may skip this step
  if the latest version of R is already installed on your system. If R
  is not installed on your system, it may either be installed locally
  (e.g., in an individual user's \texttt{bin} directory) or globally
  (e.g., in the \texttt{/bin} directory). The latter requires
  administrative privileges. In either case, the latest release of R
  may be downloaded from the CRAN website
  (\href{http://cran.r-project.org}{http://cran.r-project.org}).

\item {\bf Installing MNP}. 

\begin{enumerate}
\item Create a local library directory if it does not exist already.
  Here, we use \texttt{\~{}/Rlib/library} but you can specify a
  different directory. This directory can be created by typing the
  following command at the command prompt,
\spacingset{1}\begin{verbatim}
       mkdir ~/Rlib ~/Rlib/library
\end{verbatim}
\spacingset{1.5}
  
\item Open the \texttt{\~{}/.Renviron} file in your home directory (or
  create it if it does not exist) and add the following line,
\spacingset{1}\begin{verbatim}
       R_LIBS="~/Rlib/library"
\end{verbatim}
\spacingset{1.5}
  Alternatively, one can define the environmental variable. For
  example, add the following line to your Bourne shell startup file
  (e.g., \texttt{.bashrc} file if you are using a bash shell),
\spacingset{1}\begin{verbatim}
       export R_LIBS="$HOME/Rlib/library"
\end{verbatim}
\spacingset{1.5} 

\item Start R and type at the prompt:
\spacingset{1}\begin{verbatim}
       install.packages("MNP", lib="~/Rlib/library/")
\end{verbatim}
\spacingset{1.5}
\end{enumerate}
...
\subsection{MacOS X systems}

\begin{enumerate}
\item {\bf Installing the latest version of R.} You may skip this step
  if the latest version of R is already installed on your system. If R
  is not installed on your system, you may download it from the CRAN
  website
  (\href{http://cran.r-project.org}{http://cran.r-project.org}).
  
\item {\bf Installing MNP.} If you are using RAqua, typing the
  following command at the prompt,
\spacingset{1}\begin{verbatim}
       install.packages("MNP")
\end{verbatim}
\spacingset{1.5}
  will install MNP into the default local library directory,
  \texttt{\~{}/Library/R/library}.
  
  If you are using the command line R, then the installation of the
  MNP package can be done exactly in the same way as in Unix/Linux
  systems.  You might want to set \texttt{R\_LIBS} to
  \texttt{\~{}/Library/R/library} so that the command line R and RAqua
  can share the same local library directory.
\end{enumerate}
...
{\spacingset{1}
\begin{tabular}{.ll}
\multicolumn{1}{r}{version}  & date &  changes \\
  2.6-4 & 6.09.13 & a label error for latent variables (thanks to Sky Xue).\\
  2.6-3 & 12.06.11 & a minor change to eliminate warnings for R2.14-0. \\
  2.6-2  & 10.27.10 & a bug fixed in {\tt predict()} (thanks to Lane Burgette).\\
  2.6-1  & 09.23.09 & trace restriction used as the default (thanks to Lane Burgette).\\
 2.5-6  & 03.24.08 & an important bug fix for the treatment of base
  categories \\
 & & (thanks to Ryan Black and Taeyoung Park) \\
  2.5-5  & 08.01.07 & returning X matrix from {\tt predict()}\\
  2.5-4  & 05.24.07 & another bug fix in {\tt predict()} (thanks to Andrew Owen).\\
  2.5-3  & 12.04.06 & a minor change.\\
  2.5-2  & 11.30.06 & a bug fix in {\tt predict()}\\
  2.5-1  & 11.21.06 & rewrite {\tt predict()} in C for speedup. Changed
  {\tt moredraw} to {\tt n.draws}.\\
  2.4-2  & 10.17.06 & bug fix in {\tt moredraw} option (thanks to Ken Benoit).\\
  2.4-1  & 10.05.06 & added an option, {\tt moredraw}, to {\tt
    predict.mnp()} (thanks to Ken Benoit).\\
  ...
  2.3-6  & 01.11.06 & print the number of estimated parameters in {\tt
    summary()} (thanks to S.C. Wang) \\
  2.3-5  & 12.27.05 & \texttt{-1} is allowed in \texttt{formula}
  (thanks to Daeyoung Koh) \\ 
  ...
  2.3-1  & 05.27.05 & added \texttt{coef.mnp()} and \texttt{cov.mnp()} (thanks to Natasha Zharinova). \\
  2.2-3  & 05.12.05 & minor changes to the documentation; \\ & & version
  published in {\it Journal of Statistical Software} \\
  2.2-2  & 05.09.05 & minor changes to the documentation \\
  2.2-1  & 05.01.05 & stable release for R 2.1.0; The observations with missing
  values in $X$ \\ & & will be deleted in \texttt{mnp()} and \texttt{predict()} (thanks to Natasha Zharinova). \\
  2.1-2  & 03.22.05 & added an option, newdraw, for \texttt{predict()} method \\
  2.1-1  & 02.25.05 & improved \texttt{predict()} method; documentation enhanced and edited \\
  2.0-1  & 02.12.05 & added \texttt{predict()} method (thanks to Xavier Gerard and Saleem Shaik) \\
  ...
  1.1-2  & 09.14.04 & minor fix in \texttt{mnp()} (thanks to Ken Shultz) \\
  ...
  1.0-3  & 06.30.04 & bug fix in \texttt{xmatrix.mnp()} (thanks to Andrew Martin) \\
  1.0-2  & 06.29.04 & removed \texttt{p.alpha0} parameter \\
...
\begin{tabular}{.ll}
\multicolumn{1}{r}{version}  & date &  changes \\
  0.9-13 & 05.28.04 & bug fix in \texttt{ymatrix.mnp()} \\
  0.9-12 & 05.23.04 & updating the documentation and help files \\
  0.9-11 & 05.08.04 & bug fix in \texttt{cXnames()} (thanks to Liming Wang) \\
  0.9-10 & 05.03.04 & first stable version; bug fix in labeling \\
  0.9-9  & 05.02.04 & bug fix in sampling of W;\\ & & added \texttt{summary.mnp()} and \texttt{print.summary.mnp()} \\
  0.9-8  & 04.29.04 & improving sampling of W, replace \texttt{printf()} with \texttt{Rprintf()} \\
  0.9-7  & 04.27.04 & missing data allowed for all models, varying choice sets allowed. \\
  0.9-6  & 04.26.04 & missing data allowed in the response variable for standard MNP; \\ 
         &          & a major bug fixed for MoP (thanks to Shigeo Hirano) \\
  0.9-5  & 04.25.04 & improper prior handled by algorithm 1 \\
  0.9-4  & 04.21.04 & bug fix in MoP, R 1.9.0 compatible, changes in
  mprobit.R \\
  0.9-3  & 04.10.04 & \texttt{rWish()} modified with an improved algorithm \\
